      ...
      effects: [
        {
          ...
          value: {
            kind: 'clamp',
            value: ref('event', 'x'),
            min: ref('state', 'player.minX'),
            max: ref('state', 'player.maxX'),
          },
        },
      ],
    },
    {
      ...
      effects: [
        ...
        { kind: 'emit', type: 'game.itemResolved', payload: { itemId: ref('entity', 'data.id') } },
        {
          kind: 'schedule',
          delayMs: literal(500),
          effects: [{ kind: 'emit', type: 'game.resolutionFinished' }],
        },
      ],
    },
    {
      id: 'damage',
      event: 'player.hit',
      effects: [{ kind: 'increment', path: 'health', value: literal(-1), min: literal(0) }],
    },
...
describe('game outputs and lifecycle', () => {
  test('emits app-domain outputs and represents delays without platform timers', () => {
    ...
    const resolved = applyGameEvent(
      definition,
      started.session,
      { type: 'entity.resolve', entityId: target.id },
      input,
    );
