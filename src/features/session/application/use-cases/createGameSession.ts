...
export function createGameSession(definition: GameDefinition): GameSession {
  ...
  return { definitionId: definition.id, stageId: firstStage.id, phase: definition.initialPhase ?? 'playing', state: { ...(definition.initialState ?? {}), ...(firstStage.initialState ?? {}) }, entities: {}, scheduled: [], elapsedMs: 0, sequence: 0 };
}
